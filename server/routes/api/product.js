...
const AI_SEARCH_URL = process.env.AI_SEARCH_URL || 'https://charitha986-commerce-ai-searching.hf.space';
const AI_API_KEY = process.env.AI_API_KEY || 'dev-key-change-me';
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Auto-index product image in AI search service
      if (image) {
        try {
          ...
          console.log('Product indexed in AI search:', savedProduct._id);
        } catch (aiError) {
          console.log('AI indexing failed (product still saved):', aiError.message);
        }
      }
...
router.delete(
  ...
  async (req, res) => {
    try {
      // Find product first to get imageKey for Cloudinary deletion
      const productDoc = await Product.findOne({ _id: req.params.id });

      if (productDoc && productDoc.imageKey) {
        try {
          const cloudinary = require('cloudinary').v2;
          await cloudinary.uploader.destroy(productDoc.imageKey);
          console.log('Cloudinary image deleted:', productDoc.imageKey);
        } catch (cloudErr) {
...
// Visual search by image — sends image to HuggingFace AI service
router.post('/visual-search', upload.single('image'), async (req, res) => {
  try {
    ...
    // Send image to AI search service
    ...
    console.log('AI search raw results:', JSON.stringify(results));

    // Extract product_ids from AI results (only valid MongoDB ObjectIds)
    const productIds = results
      .map(r => r.product_id)
      .filter(id => id && id !== 'unknown' && Mongoose.Types.ObjectId.isValid(id));
    ...
    if (productIds.length > 0) {
      ...
      console.log('No valid product IDs from AI results');
    }

    // Attach confidence scores, filter > 50%, sort by best match, limit to top 5
    const productsWithScores = products
      .map(product => {
        const match = results.find(r => r.product_id === product._id.toString());
        const confidence = match ? (1 / (1 + match.score) * 100).toFixed(1) : 0;
        return {
          ...product.toObject(),
    ...
    console.log('Visual search error:', error.message);
    res.status(400).json({
      error: 'Visual search failed. Please try again.'
    });
  }
});

// Index product image in AI search when admin adds a product
router.post('/index-image', auth, role.check(ROLES.Admin, ROLES.Merchant), upload.single('image'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const aiResponse = await axios.post(`${AI_SEARCH_URL}/add-image`, formData, {
      headers: {
        ...formData.getHeaders(),
        'X-API-Key': AI_API_KEY,
        'Authorization': `Bearer ${HF_TOKEN}`
    ...
    res.status(200).json({
      success: true,
      message: 'Product indexed for visual search',
      totalIndexed: aiResponse.data.total_images
    });
  } catch (error) {
    console.log('Index image error:', error.message);
    res.status(400).json({
      error: 'Failed to index product image.'
    });
  }
});
